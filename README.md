...
[MIT License](LICENSE) · [現在の状況](docs/development_workflow/current_status.md) · [セットアップ](docs/setup/README.md) · [音声面談](docs/voice/README.md) · [開発手順](docs/development_workflow/README.md) · [回答の生成・校閲](docs/grounded_conversation/README.md)

> **現在は本人による受け入れ確認前の統合版です。** 2026-09-22にJEV回答処理、資料取り込み、管理設定、文字・音声の会話をmainへ統合しました。初期画面から文字か音声を選び、会話欄で質問します。動画は未実装です。本人データは配布していないため、実際にAIへ質問するには自分の環境とデータの設定が必要です。
...
- **数字や担当範囲を確認する** — JEV版は短い回答候補と根拠IDを作り、形式確認と6軸の点検を通します。legacy版は引用付きの要約・言い換えを同じAI Providerで校閲します。どちらも送信前に公開状態を再確認します。経歴概要には、元資料を確認した短い紹介文を任意で設定できます。
- **分からないことを扱う** — 回答できる範囲、不明点、質問の曖昧さを区別します。
- **開発時に応答を確認する** — 「開発者モード」は初期状態でオフです。有効にすると、会話欄の外に応答速度・検索類似度・処理回数等を表示します。
- **資料と採点設定を管理する** — `/admin/intake`で資料の取り込み・確認・承認、`/admin`でJEVの採否や音声設定を調整できます。管理APIは管理鍵による認証が必要です。
...
flowchart LR
  ...
  G --> S1[JEVで回答範囲と根拠を選別]
  S1 --> A[OpenCode Go / OpenAIで短い候補を生成]
  A --> V[形式・根拠ID・文字数の検証]
  V --> R[JEVで6軸を点検]
  R --> P[公開状態の再確認]
  P --> C[検証済み本文を表示・音声化]
...
| AI | JEV版の候補生成はOpenCode Go / OpenAI。legacy版はGemini / OpenAI / Claude / OpenCode Go。検索用EmbeddingはGemini / OpenAI / Workers AI |
| JEV | TypeSafe公式HTTPまたはWorkers AI。設定した呼び出し先を使用 |
| 音声 | Gemini STT / Streaming TTS、AudioWorklet、WebAudio。初期状態は無効 |
| 検証 | Node.jsのテスト、Playwright＋Google Chrome |

上図は`ANSWER_PIPELINE=jev_v1`の経路です。通常は選別・生成・点検を行い、設定と残り時間に応じて追加探索や修復を行います。同じ材料での全文再生成は最大1回です。資料不足の定型案内は生成を省きます。[JEV版の設定と制約](docs/grounded_conversation/jev.md)を参照してください。共有テンプレートと未指定時は`legacy`で、[従来の回答生成・校閲](docs/grounded_conversation/README.md)も利用できます。検証前の候補は表示・音声化しません。
...
「開発者モード」で完了した回答の詳細を開いて確認します。表示の切り替えによる追加のAI処理・検索・DB問い合わせはありません。数値は完了イベントの付加情報で、回答本文・会話履歴・音声読み上げには混ぜません。表示状態と数値は画面のメモリだけに保持し、再読み込みでリセットします。指標は検索時点の値であり、現在の根拠の公開状態を証明するものではありません。
...
**実際にAIで回答するには**、CloudflareのWorkers・D1・Vectorizeと、選んだ回答・検索Providerの設定が必要です。JEV版にはOpenCode GoまたはOpenAIの設定と、JEVの呼び出し先の設定も必要です。非表示規則がない場合も`USER_CONTENT_EXCLUSIONS`を明示します。[セットアップ手順](docs/setup/README.md)に、Providerの選択、リソース作成、Secret登録、データ承認・投入、実API評価をまとめています。APIの利用料は各サービスの条件に従います。
...
| 確認 | 結果・範囲 |
| --- | --- |
| コアテスト | 506件通過。回答・認証・公開状態・資料取り込み・音声など |
| ブラウザテスト | 全100件を実行し94件通過、6件は古いfixtureで失敗。fixture修正後に該当分を再実行し、100件すべての成功を確認。修正後の全件一括再実行はしていません |
| 型チェック・Workerビルド | 通過 |

ブラウザテストは既存のGoogle Chromeを使用します。テスト用の架空データと通信の置き換えを使うため、上記のテストで実AI APIは呼びません。今回、新たな実API評価は行っていません。実際の回答品質は、本人が確認したデータと評価セットで別途検証します。テスト通過や校閲合格は、すべての回答の正しさを保証するものではありません。
...
- クライアントからは直近6往復・合計5,500文字までを会話の文脈として送ります。JEVの選別・点検では直近2往復を使い、合計3,600字を超えたら直近1往復に絞ります。停止・失敗した回答は次の質問の履歴から除きます。
...
ローカルの記録用proxyを使う検証では例外として、会話本文、文字起こし、マイク音声、受信した回答音声、進行イベントを `.local/conversation-recordings/` へ保存します。保存済みファイルは会話終了や再読み込みでは消えません。初回の記録状態確認に失敗すると送信を止めます。音声面談中の保存失敗は表示し、記録を止めて面談は続けます。未送信の末尾が欠ける場合があります。録画映像の保存機能はありません。[記録方法と制約](docs/voice/README.md#ローカル検証の会話保存)を確認してください。
...
JEV版は候補本文・回答可能範囲・根拠IDを扱い、対象一致、項目一致、根拠支持、因果、範囲、不要な棄権の6軸を点検します。legacy版は引用付きの要約・言い換えを同じAI Providerで校閲します。どちらも検証と公開状態の再確認が済んだ回答だけを表示・音声化します。
...
採点設定・資料取り込みの管理画面と、ローカル検証用の会話・音声記録は実装済みです。Notionの自動同期、質問箱、Feedback、公開アプリの永続会話ログ、本人声の再現、映像は未実装です。会話・下書きの保持や狭い画面の操作に改善課題が残ります。[現在の状況](docs/development_workflow/current_status.md)と[音声の未完了項目](docs/voice/README.md#phase-2の残る確認)を参照してください。
