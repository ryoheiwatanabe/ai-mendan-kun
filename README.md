...
- **数字や担当範囲を保持する** — 通常の事実回答は原文の段落と照合します。経歴概要には、元資料と照合した短い紹介文を任意で設定できます。
...
続きの質問では、直前の質問と回答を検索の手掛かりに使う場合があります。履歴自体を事実の根拠にはしません。一部だけ回答できた場合の数値は、回答した部分の根拠だけが対象です。Exact FactsやAIによる解釈だけの回答、対応するスコアのない回答、情報不足・曖昧な質問への案内は「算出対象外」と表示し、便宜的な0%や100%は付けません。停止・失敗・生成途中の回答には指標を表示しません。
...
通常の事実回答は承認済みの完全な段落から選びます。全体の経歴紹介には、事前にAIが校閲した短い派生文を任意で使えます。派生文は本人承認済みの正本とは区別し、元資料の更新・追加・撤回で失効します。[設定と制約](docs/career_overview/README.md)を参照してください。名前の確認には、同じ承認済み見出しと固有名詞一覧で照合できる名前を短く返します。一般の質問への自由な言い換えは未対応で、原文の回答が長く硬くなることはあります。
