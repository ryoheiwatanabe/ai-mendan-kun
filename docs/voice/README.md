...
- 言語パックは、案内を読んだ利用者が「日本語の言語パックを追加する」を押した時だけ`install()`を呼びます。自動ではダウンロードしません。
- 自動の聞き分けが使えない場合、端末内・クラウド認識では認識器が聞き続け、利用者が「発言を送る」で区切ります。従来方式は録音ボタンで区切ります。
- 認識を続けられない失敗は、面談の完了とは区別し、理由を示して方式を選び直してもらいます。失敗した方式は選び直しの候補から外し、黙って別の方式へは切り替えません。
...
5. 少数試行なのでP50・P95を主張に使わず、誤りは元の音声と確定文字を並べて人が判定します。
