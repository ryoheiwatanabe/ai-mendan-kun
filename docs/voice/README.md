...
- 発話ON/OFF（「AIの読み上げ（オフは文字だけ）」）は、AIの読み上げだけを切り替える。マイクの使用とは独立で、手入力に切り替えても設定は変わらない。オフにすると再生中の読み上げも止める。
- テキスト入力はいつでも使える。音声で始めた会話でも、同じ会話の中で手入力へ切り替えられる。会話履歴と発話の設定は保たれる。
- 聞き取りの開始・一時停止を本人の操作で行える（「聞き取りを止める」／「聞き取りを再開」）。音声を選んだだけで録音は始めない。
- 音声認識の方式は補助設定にまとめ、既定は自動で選ぶ。入口で方式を選ばせない。
- マイクの拒否・音声認識の失敗後も、同じ画面の「マイクを使わず文字入力で続ける」から文字で会話を続けられる。
...
- 読み上げの速さは`VOICE_PLAYBACK_RATE`（既定1。0.75〜1.5に丸める）で変えられます。1以外はWebAudioの`playbackRate`を使うため、**音の高さも変わります**（テープを速めるのと同じ）。1.2は約2割速く、その分だけ声は高くなります。
- 長い回答でも最初の音声が早く出るように、最初の分割だけ80字以内の文の区切りで切ります（以後は180字）。続きの生成は前の塊の送信後に行うため、塊の境目で少し待つことがあります。
- 使用中のマイク名（音声トラックの`label`）を画面に表示します。処理場所（端末内・外部）とは別の情報で、マイクの許可の後だけ取得できます。
- 回答APIへ接続できない場合もマイクは止めず、理由を示して聞き取りを続けます。2回続けて失敗したときは、画面の再読み込みを案内します。送信の取り消しや新しい発話による中断は、失敗として表示しません。
...
ホームの「音声はこちら」から`/voice`を開きます。リンクは設定の有効・無効にかかわらず表示され、無効な環境では音声面談を開始できません。HTTPSまたはlocalhost上のマイク・AudioWorklet対応ブラウザーが必要です。`/voice`だけマイク使用を許可するPermissions-Policyを設定しています。

新たな費用や公開範囲に影響する本番有効化は、管理者が確認してから行います。この文書の各検証結果は実施時点の記録です。統合版の配信・受け入れ状況は[現在の状況](../development_workflow/current_status.md)を参照してください。
...
マイク音声は約1秒ごとのpartを保存し、終了時に結合します。ブラウザーの強制終了や通信切断では、未送信の末尾や終了通知が欠ける場合があります。終了通知が欠けた場合は、停止意図を受信できた範囲で `microphone.partial.webm`／`.mp4` と `partial-manifest.json` を作り、不完全な記録として残します。停止意図も届かない場合でも受信済みpartは残ります。初回の記録状態確認に失敗すると送信を止めます。音声面談中の保存失敗では記録を止めてエラーを表示し、面談は続けます。
