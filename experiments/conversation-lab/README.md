...
## 手動テスト画面（2026-09-18追加）

起動後、[手動テスト画面](http://127.0.0.1:8788/manual)を開きます。既存の起動方法・キー取得方法を使います。

- 対象は架空人物のミナト（fictional-minato）。質問を自由入力するか、履歴付きの架空ケースを選べます。
- Bは現行検索＋短い指示で1回生成。Cは同じ検索コードで得た根拠を固定し、現行の生成・機械確認・校閲・修復を実行します。全体の本番経路を再現するものではありません。
- 「送信内容を確認」で対象・質問・履歴・送信先・モデル・呼び出し上限を表示し、確認後に一問ずつ実行します。Bは最大1回、Cは手動画面側の上限6回、待ち時間の上限は60秒。Cは現行処理に必要な回数だけ呼び、HTTP失敗も試行数に含めます。
- 回答本文、待ち時間、検索・生成・校閲の時間、API試行数、使用した根拠を表示します。結果は `.local/conversation-lab/manual.jsonl` に保存し、Dの評価記録と分けます。
- JEVは保存済みの新形式の比較結果を閲覧できます。画面からのJEV実行・回答採否への適用・自動修復との接続は未実装です。旧parserの記録は混ぜません。
- ベクトル検索はローカル代用で、本番と同じ精度・速度ではありません。実モデルの回答品質と接続状態は、利用者が実行して確認します。

起動・表示・計画確認・結果閲覧ではモデルAPIを呼びません。サーバーは127.0.0.1限定、モデル送信先はOpenCode固定です。外部サイトからのPOSTを拒否し、確認済み計画は一度だけ実行できます。本人資料・実会話・秘密情報は入力しないでください。

従来のA専用画面は `/` に残しています。以下のPhase 1の画面説明はA専用画面のものです。
...
- Dの画面からの実行（CLIは実装済み、画面は保存結果の閲覧のみ）。
- E（ルーティング比較）。
- 比較結果の自動集計と再試験用の人手採点画面。
