...
export function validateSegment(segment: Segment, evidence: Evidence[], allowInterpretation = false, question = ""): { ok: boolean; text?: string; reason?: string; matchedEvidenceIds?: string[] } {
  ...
  if (segment.kind === "name") {
    ...
    return { ok: true, text: `${text}です。`, matchedEvidenceIds: matched.map(item => item.id) };
  }
  if (segment.kind === "fact") {
    // 本文の事実は否定・但し書きを切り落とさないよう全文単位で照合する。
...
export function parseSegment(value: unknown): Segment {
  ...
  if ((item.kind !== "fact" && item.kind !== "name" && item.kind !== "interpretation") || typeof item.text !== "string" || !Array.isArray(item.evidenceIds) || item.evidenceIds.some(id => typeof id !== "string")) throw new Error("invalid_model_segment");
