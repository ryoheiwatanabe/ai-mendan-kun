...
表示文に「本人の確認が必要」「未確定」など、根拠にない不足の説明だけを加える場合は、kind:"limitation"のclaimとし、supportsを空配列で返します。limitationはmissing-info/needsDecisionの説明に限定し、数値・氏名・約束や、肯定・否定を問わず本人の事実の断定を含めてはいけません。limitation以外のclaimはsupportsを1件以上持たせます。各segmentには、kind:"statement"でsupportsが1件以上あるclaimを最低1つ含めてください。limitationしか無い回答は禁止です。
