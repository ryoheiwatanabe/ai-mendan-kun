...
import { EntryChoices } from "./entry-choices.tsx";

// 入口の選択と会話画面の開始状態を、同じ画面で扱う。
export function Home({ processors }: { processors: string }) {
  ...
  return <main className="shell">
    ...
    <div className="workspace">
      <aside className={started ? "intro intro-active" : "intro"}>
        <div className="intro-main"><p className="intro-caption">面談までの、もうひとつの対話。</p><h1>会う前に、<br />少し話そう。</h1><p className="intro-copy">経歴だけでは、伝わらないこと。<br />任せたい仕事や、気になる考え方。<br />本人の記録をもとに、AIがお答えします。</p><EntryChoices onText={() => setStarted(true)} /></div>
        <div className="intro-note"><span className="note-mark" aria-hidden="true">＊</span><p>ここで見つかった疑問が、<br />当日の会話のきっかけになれば。</p></div>
      </aside>
      <Chat processors={processors} started={started} onStarted={() => setStarted(true)} onEnded={() => setStarted(false)} />
    </div>
