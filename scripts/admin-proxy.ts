...
type Admin = { prepare(value: unknown): Promise<any>; approve(value: unknown, hash: string): Promise<any>; revoke(id: string): Promise<any>;
  checkProvider(): Promise<any>; reembed(): Promise<any> };
export default {
  async fetch(request: Request, env: { KNOWLEDGE_ADMIN: Admin }) {
    ...
    try {
      ...
      const result = input.action === "stage" ? await env.KNOWLEDGE_ADMIN.prepare(input.bundle)
        ...
        : input.action === "reembed" ? await env.KNOWLEDGE_ADMIN.reembed()
